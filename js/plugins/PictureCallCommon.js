 ...
 *
 * 新しく作ったコモンイベントからピクチャボタン化をするには
 *
 *  ピクチャのボタン化a or
 *  P_CALL_CEa [ピクチャ番号] [コモンイベントID] [トリガー] [透明色を考慮]: 
 * 
 *  ピクチャのボタン化解除 or
...
(function() {
    ...
    Game_Interpreter.prototype.pluginCommand = function(command, args) {
        ...
        switch (getCommandName(command)) {
            ...
            case 'ピクチャのボタン化A':
                pictureId   = getArgNumber(args[0], 1, $gameScreen.maxPictures());
                commonId    = getArgNumber(args[1], 1, $dataCommonEvents2.length - 1);
                trigger     = getArgNumber(args[2], 1, 12);
                transparent = (args.length > 3 ? getArgBoolean(args[3]) : null);
                hantei = 2;
                $gameScreen.setPictureCallCommon(pictureId, commonId, trigger, transparent);
    ...
    Game_Map.prototype.setupPictureCommonEvent = function() {
        ...
        if(hantei == 1){
            //$gameMessage.add("表示する文章1")
            var event    = $dataCommonEvents[commonId];
        }
        else if(hantei == 2){
            //$gameMessage.add("表示する文章2")
            var event    = $dataCommonEvents2[commonId];
        }
    ...
    Game_Troop.prototype.setupPictureCommonEvent = function() {
        ...
        if(hantei == 1){
            //$gameMessage.add("表示する文章3")
            var event    = $dataCommonEvents[commonId];
        }
        else if(hantei == 2){
            //$gameMessage.add("表示する文章4")
            var event    = $dataCommonEvents2[commonId];
        }
